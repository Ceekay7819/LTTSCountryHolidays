...
using HolidaysCalculator.API.Cache;
using HolidaysCalculator.API.Enums;
using HolidaysCalculator.API.Models;
...
namespace HolidaysCalculator.API.Clients
{
    public class ClientService : IClientService
    {
        ...
        private readonly IHolidaysCache _cache;

        /// <inheritdoc />
        public ClientService(HttpClient httpClient, IHolidaysCache cache)
        {
        ...
        {
            var holidays = new List<HolidaysModel>();
            ...
            foreach (var countryCode in CountryCodes)
            {
                ...
                var publicHolidaysClientModels = JsonConvert.DeserializeObject<List<PublicHolidayModel>>(publicHolidays);

                holidays.Add(new HolidaysModel
                {
                    CountryCode = countryCode,
                    PublicHolidayModels = publicHolidaysClientModels
                });
            }
        ...
        public async Task<string> GetMonthWithMostHolidays(int year)
        {
            ...
            foreach (var countryCode in CountryCodes)
            {
                ...
                var publicHolidayClientModels = JsonConvert.DeserializeObject<List<PublicHolidayModel>>(publicHolidays);
